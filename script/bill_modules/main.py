def bill_digit_module_(list_of_images,hospital_id,claim):
   
    from bill_modules.ocr import ocr
...
#             ocr_rules.append(f'Dynamic OCR called for {image}')
    ...
    for tables in pages:
        ...
        tables1=[]
        mdl,vectorizer=model_mapper[ocr_selection[list_of_images[page_cnt]]]
        page_cnt+=1
        table_type_dict={}
        cnt=0
        for table in tables:
        ...
        for df in tables:
            ...
            df=dfa
            dfa.to_csv(f'/home/varuns/work_dir/bill_extractor/apollo_bill_output/{claim}_{list_of_images[page_cnt-1]}.csv',index=False)
            del dfa
